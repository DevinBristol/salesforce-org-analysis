...
async function submitTask() {
    ...
    try {
        ...
        const healthResponse = await fetch(`http://localhost:${options.port}/health`);
        if (!healthResponse.ok) {
            throw new Error('Autonomous system is not running. Start it with: npm run start');
        }
        ...
        if (result.success) {
            ...
            if (result.deploymentStatus) {
                console.log(chalk.blue('\nDeployment:'));
                console.log(`  Deployed: ${result.deploymentStatus.deployed ? 'YES' : 'NO'}`);
        ...
        if (error.message.includes('ECONNREFUSED')) {
            ...
            console.log(chalk.yellow('Please start the system first with: npm run start'));
        } else {
            console.error(chalk.red(`\nError: ${error.message}`));
        }
