...
async function deployToProduction() {
    ...
    console.log(chalk.yellow('This will deploy changes to your PRODUCTION Salesforce org.'));
    ...
    try {
        ...
        if (!confirm2) {
            console.log(chalk.yellow('Please get approval before deploying to production.'));
            return;
        }
        ...
        const { confirmFinal } = await inquirer.prompt([
            {
                ...
                validate: (input) => input === 'DEPLOY TO PRODUCTION' || 'Type exactly: DEPLOY TO PRODUCTION'
            }
        ]);

        // Run validation first
        const validateSpinner = ora('Running validation against production...').start();
        ...
        try {
            ...
            if (result.result.status === 'Succeeded') {
                ...
                console.log(chalk.gray('Deployment logged to: ./deployments/production-log.json'));
            ...
            if (rollback) {
                console.log(chalk.yellow('Please use Salesforce Setup UI to rollback the deployment.'));
            }
        }
