...
export class TestQualityAnalyzer {
    ...
    analyzeBulkTesting(content) {
        ...
        const hasBulkTest = content.includes('200') ||
        ...
        if (!hasBulkTest) {
            issues.push({
                ...
                description: 'No bulk testing detected. Tests should verify governor limit handling with 200+ records.',
                recommendation: 'Add test methods that create and process 200+ records to verify bulkification.'
            });
            score = 0;
        } else {
            score = 20;
        ...
        const testMethods = content.match(/@isTest\s+static\s+void\s+\w+\s*\(\)/g) || [];
        const bulkTestMethods = content.match(/\b(bulk|batch|200|governor)/gi) || [];
        ...
        if (testMethods.length > 0 && bulkTestMethods.length === 0) {
            issues.push({
                ...
                recommendation: 'Add test methods with "bulk" or "batch" naming to clearly indicate bulk testing.'
            });
    ...
    analyzeAssertions(content) {
        ...
        const systemAssertEqualsCount = (content.match(/System\.assertEquals\(/g) || []).length;
        const systemAssertNotEqualsCount = (content.match(/System\.assertNotEquals\(/g) || []).length;
        const assertCount = (content.match(/Assert\./g) || []).length; // New Assert class (API 56+)

        const totalAssertions = systemAssertCount + systemAssertEqualsCount +
                               systemAssertNotEqualsCount + assertCount;

        // Check for meaningless assertions
        const meaninglessAssertions = content.match(/System\.assert\s*\(\s*true\s*\)/g) || [];
        if (meaninglessAssertions.length > 0) {
            issues.push({
                ...
                recommendation: 'Replace System.assert(true) with meaningful assertions that validate actual outcomes.'
            });
            score = 0;
        }
        ...
        if (totalAssertions === 0) {
            issues.push({
                severity: 'critical',
                type: 'No Assertions',
                description: 'Test class contains no assertions. Tests must validate outcomes.',
                recommendation: 'Add System.assertEquals/assertNotEquals to verify expected behavior.'
            });
            score = 0;
        } else {
            // Score based on assertion density (assertions per test method)
            const testMethods = (content.match(/@isTest/g) || []).length;
            const assertionsPerMethod = testMethods > 0 ? totalAssertions / testMethods : 0;
            ...
            if (assertionsPerMethod < 1) {
                issues.push({
                    ...
                    recommendation: 'Add more assertions to validate all aspects of functionality.'
                });
                score = 10;
        ...
        const assertionsWithMessages = content.match(/System\.assert(?:Equals|NotEquals)?\s*\([^,)]+,[^,)]+,[^)]+\)/g) || [];
        const assertionMessagePercentage = totalAssertions > 0
        ...
        if (assertionMessagePercentage < 50 && totalAssertions > 0) {
            issues.push({
                ...
                recommendation: 'Add descriptive messages to assertions for easier debugging when tests fail.'
            });
        }
    ...
    analyzeTestDataPractices(content) {
        ...
        if (hardcodedIds.length > 0) {
            issues.push({
                ...
                recommendation: 'Remove hardcoded IDs. Create test records dynamically or query by unique field.'
            });
            score = 0;
        } else {
            score += 10;
        ...
        const hasTestSetup = content.includes('@testSetup') || content.includes('@TestSetup');
        if (!hasTestSetup && content.includes('@isTest')) {
            issues.push({
                severity: 'medium',
                type: 'No @testSetup Method',
                description: 'Consider using @testSetup for shared test data to improve performance.',
                recommendation: 'Add @testSetup method to create common test data once for all test methods.'
            });
        ...
        const hasStartStopTest = content.includes('Test.startTest') && content.includes('Test.stopTest');
        if (!hasStartStopTest) {
            issues.push({
                severity: 'high',
                type: 'Missing Test.startTest/stopTest',
                description: 'Test.startTest() and Test.stopTest() are not used. Governor limits won\'t reset.',
                recommendation: 'Wrap test execution in Test.startTest/stopTest to reset governor limits.'
            });
        } else {
            score += 10;
        }
        ...
        const hasSeeAllData = content.match(/@isTest\s*\(\s*SeeAllData\s*=\s*true\s*\)/i);
        if (hasSeeAllData) {
            issues.push({
                severity: 'critical',
                type: 'SeeAllData=true Usage',
                description: 'SeeAllData=true makes tests dependent on org data. Tests will be flaky.',
                recommendation: 'Remove SeeAllData=true and create all necessary test data explicitly.'
            });
            score = 0;
        } else {
            score += 5;
    ...
    analyzeErrorHandling(content) {
        ...
        const hasNegativeTests = content.match(/try\s*\{[\s\S]*?\}\s*catch\s*\(/g) || [];
        ...
        if (!hasExpectedExceptions) {
            issues.push({
                ...
                recommendation: 'Add test methods that verify exception handling for invalid inputs and error conditions.'
            });
            score = 0;
        } else {
            score = 15;
        ...
        const hasEmptyTests = content.match(/\.isEmpty\(\)|\.size\(\)\s*==\s*0|new List</g) || [];

        if (hasNullTests.length === 0 || hasEmptyTests.length === 0) {
            issues.push({
                severity: 'medium',
                type: 'Missing Edge Case Tests',
                description: 'Tests don\'t appear to cover null values or empty collections.',
                recommendation: 'Add tests for edge cases: null inputs, empty lists, boundary values.'
            });
        } else {
            score += 5;
        }
    ...
    analyzeMockingUsage(content) {
        ...
        const needsMocking = content.includes('HttpCallout') ||
        ...
        if (needsMocking) {
            ...
            const hasMocking = content.includes('HttpCalloutMock') ||
            ...
            if (!hasMocking) {
                issues.push({
                    ...
                    description: 'Test involves external dependencies but doesn\'t use mocking.',
                    recommendation: 'Implement HttpCalloutMock or StubProvider to isolate test from external dependencies.'
                });
                score = 0;
            } else {
                score = 15;
    ...
    analyzeTestMethodCoverage(content) {
        ...
        // Count test methods
        const testMethods = content.match(/@isTest\s+(?:static\s+)?void\s+\w+/g) || [];
        ...
        if (testMethodCount === 0) {
            ...
            issues.push({
                ...
                description: 'Only one test method. Insufficient coverage of scenarios.',
                recommendation: 'Add separate test methods for positive, negative, and edge cases.'
            });
            score = 5;
            ...
            issues.push({
                ...
                recommendation: 'Add test methods for: bulk operations, error cases, edge conditions, security.'
            });
            score = 10;
        } else {
            score = 15;
        ...
        const positiveTests = testMethods.filter(m => m.match(/success|should|valid|correct/i));
        const negativeTests = testMethods.filter(m => m.match(/fail|error|invalid|exception/i));
        const bulkTests = testMethods.filter(m => m.match(/bulk|batch|governor|200/i));

        if (positiveTests.length === 0 && testMethodCount > 0) {
            issues.push({
                severity: 'low',
                type: 'Unclear Test Naming',
                description: 'Test method names don\'t clearly indicate positive test cases.',
                recommendation: 'Use descriptive names like testValidInputSuccess() for clarity.'
            });
        }
    ...
    analyzeDocumentation(content) {
        ...
        if (!hasClassDoc) {
            issues.push({
                ...
                recommendation: 'Add class-level JavaDoc describing what functionality is being tested.'
            });
        } else {
            score += 5;
        }
        ...
        const testMethods = content.match(/@isTest\s+(?:static\s+)?void\s+\w+/g) || [];
        const documentedMethods = content.match(/\/\*\*[\s\S]*?\*\/\s*@isTest/g) || [];

        const docPercentage = testMethods.length > 0
        ...
        if (docPercentage < 50) {
            issues.push({
                ...
                recommendation: 'Add JavaDoc to test methods describing: Given/When/Then or scenario being tested.'
            });
        } else {
            score += 5;
        }
    ...
    calculateQualityScore(metrics) {
        const weights = {
            bulkTesting: 0.20,      // 20% - Critical for governor limits
            assertions: 0.20,        // 20% - Must validate outcomes
            testDataPractices: 0.20, // 20% - Data setup quality
            errorHandling: 0.15,     // 15% - Edge case coverage
            mockingUsage: 0.10,      // 10% - Isolation from dependencies
            testCoverage: 0.10,      // 10% - Number of test scenarios
    ...
    shouldRewrite(analysisResult) {
        return analysisResult.score < 40 ||
