...
class HealthChecker {
    ...
    async checkSalesforceCLI() {
        ...
        try {
            ...
            spinner.fail('Salesforce CLI not found (install: npm install -g @salesforce/cli)');
            this.failed++;
        }
    }
    ...
    async checkSalesforceAuth() {
        ...
        try {
            ...
            const hasProduction = result.result.nonScratchOrgs?.some(org => 
                org.alias === 'production' || org.isDefaultUsername
            );
            const hasSandbox = result.result.nonScratchOrgs?.some(org => 
                org.alias === 'dev-sandbox'
            );
            ...
            if (hasProduction || hasSandbox) {
                ...
                spinner.warn('No Salesforce orgs authenticated (run: sf org login web)');
                this.failed++;
            }
        } catch (error) {
    ...
    displayResults() {
        console.log(chalk.cyan.bold('\n╔══════════════════════════════════════════════════════╗'));
        console.log(chalk.cyan.bold('║     Health Check Results                            ║'));
        console.log(chalk.cyan.bold('╚══════════════════════════════════════════════════════╝'));
