...
export class TaskAnalyzer {
    ...
    async analyze(description) {
        try {
            ...
            const result = TaskAnalysisSchema.safeParse(rawData);
            if (!result.success) {
                this.logger.error('Schema validation failed:', result.error.errors);
            ...
            return this.enhanceAnalysis(analysis, description, usage);
        } catch (error) {
...
Return your analysis as JSON with this structure:
{
    ...
    basicAnalysis(description) {
        ...
        else if (desc.includes('apex') || desc.includes('class') || desc.includes('trigger')) type = 'apex';
        else if (desc.includes('object')) type = 'object';
        else if (desc.includes('integration') || desc.includes('api')) type = 'integration';
        else if (desc.includes('report') || desc.includes('dashboard')) type = 'report';
    ...
    extractObjects(description) {
        const objects = [];
        const commonObjects = ['Account', 'Contact', 'Lead', 'Opportunity', 'Case', 'Task', 'Event'];
    ...
    enhanceAnalysis(analysis, originalDescription, usage = null) {
        ...
        this.logger.info(`Task analyzed - Type: ${enhanced.type}, Risk: ${enhanced.riskLevel}`);
