#!/usr/bin/env node
// setup-phase2-auto.js - Non-Interactive Phase 2 Setup Script
...
class Phase2Setup {
    constructor() {
        ...
        this.config = {
            ...
            slackChannel: process.env.SLACK_CHANNEL || '#salesforce-dev',
            workerCount: parseInt(process.env.WORKER_COUNT) || 3,
            maxConcurrency: parseInt(process.env.MAX_CONCURRENCY) || 5,
            costBudget: parseInt(process.env.COST_BUDGET_MONTHLY) || 100,
            useOpusPlanning: process.env.USE_OPUS_PLANNING === 'true',
            dbPath: process.env.DB_PATH || './phase2/state.db',
        };
        this.newFiles = [];
        this.modifiedFiles = [];
    }
...
╚══════════════════════════════════════════════════════════╝
    `));
        ...
        console.log(`  Slack Channel: ${this.config.slackChannel}`);
        console.log(`  Worker Count: ${this.config.workerCount}`);
        console.log(`  Cost Budget: $${this.config.costBudget}/month`);
        console.log(`  Opus Planning: ${this.config.useOpusPlanning ? 'Enabled' : 'Disabled'}\n`);
        ...
        try {
            ...
            await this.createMainEntryPoint();

            // Step 10: Create worker script
    ...
    async verifyCredentials() {
        const spinner = ora('Verifying Slack credentials...').start();

        if (!this.config.slackToken) {
            throw new Error('SLACK_BOT_TOKEN not found in .env');
        }
        if (!this.config.slackAppToken) {
            throw new Error('SLACK_APP_TOKEN not found in .env');
        }
        if (!this.config.slackSigningSecret) {
            throw new Error('SLACK_SIGNING_SECRET not found in .env');
        }

        spinner.succeed('Slack credentials verified');
    }
    ...
    async installDependencies() {
        ...
        try {
            await execAsync(`npm install ${dependencies.join(' ')}`, { cwd: '.' });
...
export class SlackBot {
  ...
  setupEvents() {
    ...
    this.app.event('app_mention', async ({ event, say }) => {
      await say(\`Hello <@\${event.user}>! Use /sf-dev status to check system status.\`);
    });
  }
  ...
  async handleDeploy(say, taskId) {
    await say(\`Deploying task \${taskId}...\`);
...
export class Dashboard {
  ...
  log(message) {
    ...
    async modifyExistingServices() {
        const spinner = ora('Checking existing services...').start();

        // Note: Skipping modifications to avoid breaking existing system
        // These can be added later if needed

        spinner.succeed('Existing services preserved');
    }
    ...
    async configureSlackBot() {
        ...
        // Create Slack app manifest for reference
        const manifest = {
            display_information: {
  ...
  apps: [{
    ...
    time: true
        ...
        await fs.writeFile('./ecosystem.config.js', ecosystem);

        spinner.succeed('PM2 configured');
    }

    async createMainEntryPoint() {
        const spinner = ora('Creating main entry point...').start();

        const code = `// src/phase2/main.js
import dotenv from 'dotenv';
import { WorkerPool } from './worker-pool.js';
...
async function main() {
  console.log('🚀 Starting Phase 2 Multi-Agent System...');

  // Initialize worker pool
  const workerPool = new WorkerPool(parseInt(process.env.WORKER_COUNT) || 3);
  await workerPool.start();
  console.log(\`✅ Started \${process.env.WORKER_COUNT} workers\`);

  // Initialize Slack bot
  const slackBot = new SlackBot(workerPool);
  await slackBot.start();
  console.log('✅ Slack bot connected');

  // Initialize dashboard (optional)
  if (process.env.ENABLE_DASHBOARD === 'true') {
    const dashboard = new Dashboard(workerPool);
    dashboard.init();
    console.log('✅ Dashboard initialized');
  }

  console.log('\\n🎉 Phase 2 system is running!');
  console.log('Use /sf-dev status in Slack to check system status');
}

main().catch(error => {
  console.error('❌ Failed to start Phase 2 system:', error);
  process.exit(1);
});`;

        await fs.writeFile('./src/phase2/main.js', code);
        this.newFiles.push('main.js');

        spinner.succeed('Main entry point created');
    }

    async createWorkerScript() {
        const spinner = ora('Creating worker script...').start();

        const code = `// src/phase2/worker.js
...
async function improveTest(task) {
  ...
  return { improved: true, message: 'Test improved' };
}

async function generateTest(task) {
  // Placeholder implementation
  return { generated: true, message: 'Test generated' };
}

async function analyzeCode(task) {
  // Placeholder implementation
  return { analyzed: true, message: 'Code analyzed' };
}

console.log(\`Worker \${workerData.workerId} initialized\`);`;

        await fs.writeFile('./src/phase2/worker.js', code);
        this.newFiles.push('worker.js');

        spinner.succeed('Worker script created');
    }

    displayNextSteps() {
        console.log(chalk.cyan('\n📋 Next Steps:\n'));
        console.log('1. Start the system: npm run phase2:start');
        console.log('2. View dashboard: npm run phase2:dashboard');
        console.log('3. Test Slack bot: /sf-dev status in Slack');
        console.log('\n' + chalk.yellow('Commands to add to package.json:'));
        console.log('  "phase2:start": "node src/phase2/main.js"');
        console.log('  "phase2:dashboard": "node src/phase2/main.js --dashboard"');
        console.log('  "phase2:stop": "pm2 stop phase2-main"');
