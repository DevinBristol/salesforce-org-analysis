...
class BatchCodebaseAnalyzer {
    ...
    async run(options = {}) {
        ...
        console.log(chalk.cyan.bold('║     Batch Codebase Analysis (No Deployment)          ║'));
        console.log(chalk.cyan.bold('╚══════════════════════════════════════════════════════╝\n'));
        ...
        try {
            ...
            if (resumeFromProgress && await fs.pathExists(this.progressFile)) {
                ...
                console.log(chalk.yellow(`Resuming from previous run: ${processedClasses.size} classes already analyzed\n`));
            }
            ...
            const coverageMap = await this.orgAnalyzer.getTestCoverage(this.salesforceManager);
            ...
            // Filter classes
            if (skipTests) {
                allClasses = allClasses.filter(c => !c.fullName.toLowerCase().includes('test'));
            }
            if (skipManaged) {
            ...
            scanSpinner.succeed(`Found ${allClasses.length} classes to analyze (${processedClasses.size} already done)`);
            ...
            for (let i = 0; i < allClasses.length; i++) {
                ...
                const spinner = ora(`${progress} Analyzing ${apexClass.fullName}...`).start();
                ...
                try {
                    ...
                    if (analyzeContent) {
                        try {
                            classContent = await this.salesforceManager.getApexClassContent(apexClass.fullName);
                        } catch (error) {
                            logger.warn(`Could not fetch content for ${apexClass.fullName}: ${error.message}`);
                        }
                    }

                    // Calculate score
                    const score = this.orgAnalyzer.calculateImprovementScore(apexClass, classContent);
                    ...
                    const complexity = classContent ?
                        ...
                        { cyclomaticComplexity: 0, cognitiveComplexity: 0, maintainabilityIndex: 0, technicalDebtMinutes: 0 };

                    // Scan for security vulnerabilities (if content available)
                    const security = classContent ?
                        this.orgAnalyzer.scanSecurityVulnerabilities(classContent, apexClass.fullName) :
                    ...
                    const result = {
                        ...
                        technicalDebtHours: Math.round(complexity.technicalDebtMinutes / 60 * 10) / 10
                    };
                    ...
                    const securityBadge = security.critical.length > 0 ? ' 🔴SEC' : security.high.length > 0 ? ' 🟠SEC' : '';
                    const coverageBadge = coverage.coverage === 0 ? ' 📊0%' : coverage.coverage < 75 ? ` 📊${coverage.coverage}%` : '';
                    spinner.succeed(`${progress} ${apexClass.fullName} - Score: ${score.total} (${score.risk})${securityBadge}${coverageBadge}`);
            ...
            console.log(chalk.gray(`Report saved to: ./analysis/org-analysis-report.md`));
            console.log(chalk.gray(`Detailed data saved to: ./analysis/org-analysis-data.json`));
    ...
    async generateOrgReport() {
        ...
        const sortedResults = [...this.analysisResults].sort((a, b) => b.score - a.score);
        ...
        const lowCoverage = sortedResults.filter(r => r.coverage > 0 && r.coverage < 75);
        const goodCoverage = sortedResults.filter(r => r.coverage >= 75 && r.coverage < 90);
        ...
        const highSecurity = sortedResults.filter(r => r.hasHighSecurity && !r.hasCriticalSecurity);

        // NEW: Complexity Groups
        const highComplexityClasses = sortedResults.filter(r => r.highComplexity);
        const totalTechDebt = sortedResults.reduce((sum, r) => sum + (r.technicalDebtHours || 0), 0);

        // NEW: API Version Groups
        const outdatedApi = sortedResults.filter(r => r.apiVersion && r.apiVersion.isOutdated);
        const deprecatedApi = sortedResults.filter(r => r.apiVersion && r.apiVersion.isDeprecated);
...
${criticalSecurity.slice(0, 10).map((r, i) => `
#### ${i + 1}. ${r.className}
${r.security.critical.map(v => `
- **${v.type}**
  - Severity: ${v.severity}
...
${zeroCoverage.slice(0, 10).map((r, i) => `${i + 1}. ${r.className} (${r.lineCount} lines, 0% coverage)`).join('\n')}
...
${lowCoverage.sort((a, b) => b.coverage - a.coverage).slice(0, 10).map((r, i) =>
...
${highComplexityClasses.slice(0, 10).map((r, i) => `
${i + 1}. **${r.className}**
...
- **Medium Priority (30-50):** ${sortedResults.filter(r => r.complexity.cyclomaticComplexity >= 30 && r.complexity.cyclomaticComplexity < 50).reduce((sum, r) => sum + (r.technicalDebtHours || 0), 0).toFixed(1)} hours
- **Low Priority (<30):** ${sortedResults.filter(r => r.complexity.cyclomaticComplexity < 30).reduce((sum, r) => sum + (r.technicalDebtHours || 0), 0).toFixed(1)} hours
...
${deprecatedApi.slice(0, 10).map(r => `- ${r.className} (API v${r.apiVersion.current})`).join('\n')}
...
${outdatedApi.slice(0, 15).map(r => `- ${r.className} (API v${r.apiVersion.current} - ${r.apiVersion.versionsBehind} versions behind)`).join('\n')}
...
### Documentation Needs
${this.getCategoryLeaders('documentation', sortedResults).map(r =>
...
`;
        ...
        await fs.writeJson('./analysis/org-analysis-data.json', {
    ...
    calculateStatistics(results) {
        ...
        return {
            ...
            maxScoreClass: results.find(r => r.score === Math.max(...scores))?.className,
            minScoreClass: results.find(r => r.score === Math.min(...scores))?.className
        };
    }
    ...
    generateRecommendations(classes, priority) {
        ...
        if (bulkIssues.length > 0) {
            recommendations.push(`- Fix bulkification issues in ${bulkIssues.length} classes (CRITICAL)`);
        }

        // Check for documentation needs
        const docIssues = classes.filter(c => c.categories.documentation >= 10);
        if (docIssues.length > 0) {
            recommendations.push(`- Add comprehensive documentation to ${docIssues.length} classes`);
        }

        // Check for error handling
        const errorIssues = classes.filter(c => c.categories.errorHandling >= 10);
        if (errorIssues.length > 0) {
            recommendations.push(`- Implement error handling in ${errorIssues.length} classes`);
        }

        // Age-based
        const oldClasses = classes.filter(c => c.categories.age >= 15);
        if (oldClasses.length > 0) {
            recommendations.push(`- Review and modernize ${oldClasses.length} outdated classes`);
        }
...
const options = {
    maxClasses: args.includes('--limit') ? parseInt(args[args.indexOf('--limit') + 1]) : null,
