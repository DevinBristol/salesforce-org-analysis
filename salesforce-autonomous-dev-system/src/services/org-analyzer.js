...
export class OrgAnalyzer {
    ...
    async analyzeOrg() {
        try {
            ...
            await fs.writeJson('./analysis/org-analysis.json', analysis, { spaces: 2 });
    ...
    generateRecommendations(summary) {
        ...
        if (summary.apexTriggers > summary.customObjects * 2) {
            recommendations.push({
                type: 'architecture',
                priority: 'medium',
                message: 'Multiple triggers per object detected - consider trigger framework'
            });
        }
        ...
        if (summary.flows > 50) {
            recommendations.push({
                type: 'governance',
                priority: 'medium',
                message: 'High number of flows - implement naming conventions and documentation'
            });
        }
    ...
    identifyIssues() {
        ...
        if (this.orgMetadata && this.orgMetadata.ApexClass) {
            ...
            const classesWithoutTest = this.orgMetadata.ApexClass.filter(
                cls => !cls.fullName.includes('Test') && 
                       !this.hasTestClass(cls.fullName)
            );
            ...
            if (classesWithoutTest.length > 0) {
                issues.push({
                    ...
                    affectedComponents: classesWithoutTest.slice(0, 5).map(c => c.fullName)
                });
            }
        }
    ...
    hasTestClass(className) {
        ...
        return this.orgMetadata.ApexClass.some(
            cls => cls.fullName === testClassName || 
                   cls.fullName === 'Test' + className
        );
    }
    ...
    async getContext(objectNames = []) {
        try {
            ...
            if (this.orgMetadata && this.orgMetadata.CustomObject) {
                context.customFields = this.orgMetadata.CustomObject
                    .filter(obj => objectNames.includes(obj.fullName.replace('__c', '')))
                    .map(obj => ({
                        object: obj.fullName,
                        label: obj.label
                    }));
    ...
    calculateImprovementScore(apexClass, classContent = null) {
        ...
        if (monthsOld > 24) {
            ...
            score.reasons.push(`Not modified for ${Math.round(monthsOld)} months - high priority for review`);
        ...
        if (classContent) {
            ...
            if (contentAnalysis.riskLevel === 'high' && score.risk !== 'high') {
                score.risk = 'high';
            } else if (contentAnalysis.riskLevel === 'medium' && score.risk === 'low') {
                score.risk = 'medium';
            }
        }

        // Calculate total
        score.total = Object.values(score.categories).reduce((sum, val) => sum + val, 0);
    ...
    async getTestCoverage(salesforceManager) {
        try {
            ...
            const coverageResults = await salesforceManager.connection.tooling.query(coverageQuery);
            ...
            this.logger.info(`Test coverage data fetched for ${Object.keys(coverageMap).length} classes`);
            return coverageMap;
        } catch (error) {
            this.logger.warn('Could not fetch test coverage:', error.message);
            this.logger.warn('Test coverage requires tests to be run at least once in the org');
            return {};
        }
    }
    ...
    analyzeClassContent(classContent) {
        ...
        if (lineCount > 500) {
            ...
            analysis.reasons.push(`Large class (${lineCount} lines) - high complexity`);
        ...
        const methodCount = (classContent.match(/\b(public|private|global)\s+(static\s+)?\w+\s+\w+\s*\(/g) || []).length;
        if (methodCount > 20) {
            analysis.complexityScore += 5;
            analysis.reasons.push(`Many methods (${methodCount}) - needs organization`);
        }

        // DOCUMENTATION (0-20 points) - Higher score = needs more docs
        const hasClassDoc = classContent.includes('/**') && classContent.indexOf('/**') < classContent.indexOf('class ');
        ...
        if (docRatio < 0.3) {
            analysis.documentationScore += 10;
            analysis.reasons.push(`Low documentation coverage (${Math.round(docRatio * 100)}%)`);
        ...
        const hasLoopDML = /for\s*\([^)]*\)\s*{[^}]*(insert|update|delete|upsert)\s+/i.test(classContent);
        ...
        if (hasLoopDML) {
            ...
            analysis.reasons.push('DML in loop detected - CRITICAL bulkification needed');
            analysis.riskLevel = 'high';
        }
    ...
    calculateComplexityMetrics(classContent) {
        ...
        const complexityFactor = metrics.cyclomaticComplexity > 50 ? 3 :
                                metrics.cyclomaticComplexity > 30 ? 2 : 1;
        const sizeFactor = loc > 500 ? 3 : loc > 300 ? 2 : 1;

        metrics.technicalDebtMinutes = Math.round((loc / 100) * complexityFactor * sizeFactor * 30);
    ...
    scanSecurityVulnerabilities(classContent, className) {
        ...
        if (content.includes('database.query(') || content.includes('[select')) {
            if (!content.includes('escapesinglequotes') && content.match(/\+.*['"`]/)) {
        ...
        for (const pattern of credentialPatterns) {
            if (content.match(pattern)) {
                vulnerabilities.critical.push({
                    ...
                    severity: 'CRITICAL',
                    recommendation: 'Use Custom Metadata, Custom Settings, or Named Credentials'
                });
                break;
            }
        }
        ...
        if (content.includes('system.debug') &&
            (content.includes('password') || content.includes('ssn') ||
        ...
        if (className.toLowerCase().includes('test') && !content.includes('@istest')) {
            vulnerabilities.high.push({
                type: 'Test Class Without @IsTest',
                description: 'Test class missing @IsTest annotation',
                severity: 'HIGH',
                recommendation: 'Add @IsTest annotation to prevent accidental deployment'
            });
        }
        ...
        if (content.includes('math.random()')) {
            vulnerabilities.medium.push({
                ...
                severity: 'MEDIUM',
                recommendation: 'Use Crypto.getRandomInteger() for security-sensitive operations'
            });
        }
        ...
        if ((className.toLowerCase().includes('controller') ||
            ...
            vulnerabilities.medium.push({
                ...
                description: 'Class lacks sharing keyword - defaults to without sharing',
                severity: 'MEDIUM',
                recommendation: 'Explicitly declare "with sharing" or "inherited sharing"'
            });
        }
